...
- **Admin Features:** Unified login with automatic panel routing, full CRUD operations on products via elegant popup/modal forms with category-specific fields (Aluminium: pricing matrix indicators and dimension notes; H-Frames: height options and quantity discounts; Cuplock: component type and length specifications; Accessories: material type and compatibility info), file upload with image preview, pricing matrix management for aluminium scaffolding products, comprehensive orders dashboard with transaction IDs for payment verification, and business analytics dashboard with interactive charts showing monthly/yearly revenue trends, order counts, and category-wise breakdown powered by Chart.js.
